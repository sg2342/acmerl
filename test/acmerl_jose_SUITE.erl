...
import_export(_) ->
    ...
    lists:foreach(
      fun(Algo) ->
        ...
        FullExportedKeyBin = iolist_to_binary(json:encode(FullExportedKey)),
        FullExportedKey2 = json:decode(FullExportedKeyBin),
        ?assertEqual(FullExportedKey, FullExportedKey2),

        {ok, ImportedKey} = acmerl_jose:import_key(FullExportedKey),
        acmerl_jose:sign(Message, ImportedKey, #{}),
...
import_examples(Config) ->
    ...
    lists:foreach(
      fun(KeyFile) ->
        ...
        {ok, Key} = acmerl_jose:import_key(json:decode(JWK)),
...
thumbprint(Config) ->
    ...
    Jwk = json:decode(FileContent),

    Thumbprint = acmerl_jose:thumbprint({jwk, Jwk}),
